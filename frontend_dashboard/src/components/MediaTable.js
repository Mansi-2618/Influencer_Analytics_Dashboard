          ...
          <table className="min-w-full text-sm border-t border-slate-700/50">
            <thead className="sticky top-0 bg-slate-900/90 backdrop-blur-sm text-slate-300 z-10">
              <tr>
                ...
                {["reach", "likes", "shares", "comments", "saves"].map((key) => (
                  <th
                    key={key}
                    onClick={() => handleSort(key)}
                    className="px-4 py-3 text-right cursor-pointer select-none hover:text-orange-400 transition-colors font-semibold group/header"
                  >
