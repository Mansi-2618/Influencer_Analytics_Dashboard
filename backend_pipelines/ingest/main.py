...
def check_account_type(user_id, pageaccess_token):
    """
    ...
    if "error" in res:
        ...
        print(f"[check_account_type] error code={error_code}: {error_msg}")
        return "PERSONAL"
...
def ingest_instagram_data(request):
    ...
    try:
        ...
        # ── Check account type via insights API ────────────────────────────────
        # Instagram does not expose account_type as a profile field.
        # Workaround: insights endpoint only works for Business/Creator accounts.
        account_type = check_account_type(IG_USER_ID, FB_ACCESS_TOKEN)
        ...
        # ── Also save account_type in processed_profile collection ─────────────
        db.collection('users_data').document(user_email).collection('processed_profile').document('summary').set({
            'account_type': account_type,   # "BUSINESS_OR_CREATOR" or "PERSONAL"
            'followers_count': profile.get('followers_count'),
            'updated_at': datetime.now(timezone.utc).isoformat(),
        }, merge=True)

        print(f"Saved account_type={account_type} to processed_profile/summary")
